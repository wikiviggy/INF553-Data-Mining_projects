...
if ( int(sys.argv[1])== 1):
    rdd=sc.textFile(sys.argv[3]).filter(lambda x: not x.startswith('user_id')).map(lambda x:x.split(',')).map(lambda x: (str(x[0]),str(x[1]))).groupByKey().map(lambda x: tuple(set(x[1])))

elif (  int(sys.argv[1])== 2):
    rdd=sc.textFile(sys.argv[3]).filter(lambda x: not x.startswith('user_id')).map(lambda x:x.split(',')).map(lambda x: (str(x[1]),str(x[0]))).groupByKey().map(lambda x: tuple(set(x[1])))  
...
candidate_itemset = rdd.mapPartitions(get_freq).distinct().collect()
result_map_phase2 = rdd.mapPartitions(lambda x: ph2(x,candidate_itemset)).reduceByKey(lambda u,v: u+v).collect()
